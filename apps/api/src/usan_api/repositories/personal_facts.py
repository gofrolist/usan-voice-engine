...
Durable categorized memory about an contact. ``create`` is used by the
``record_personal_fact`` tool (``source='contact_stated'``) and the post-call summarizer
...
# Bound the per-contact injection so a long memory never floods the next prompt. The
...
async def create(
    ...
    *,
    ...
    row = PersonalFact(contact_id=contact_id, category=category, content=content, source=source)
...
async def list_active(
    db: AsyncSession, *, contact_id: uuid.UUID, limit: int = _MAX_ACTIVE_INJECT
) -> list[PersonalFact]:
    """Active facts for an contact (oldest first) — the source of the memory built-ins."""
    stmt = (
        select(PersonalFact)
        .where(PersonalFact.contact_id == contact_id, PersonalFact.active.is_(True))
...
async def list_active_keys(db: AsyncSession, *, contact_id: uuid.UUID) -> set[tuple[str, str]]:
    """Every active (category, content) key for an contact — UNCAPPED, for extraction dedup.
    ...
    stmt = select(PersonalFact.category, PersonalFact.content).where(
        PersonalFact.contact_id == contact_id, PersonalFact.active.is_(True)
    )
