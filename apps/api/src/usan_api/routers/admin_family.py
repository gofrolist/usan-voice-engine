...
async def create_family_contact(
    ...
    # Pre-check the FK so a bad contact_id is a clean 404, not a 500 on commit.
    if await contacts_repo.get_contact(db, body.contact_id) is None:
    ...
    await admin_audit.record(
        ...
        detail={"contact_id": str(body.contact_id)},  # no name/phone (PII)
    )
    await db.commit()
...
@router.patch("/family-contacts/{family_contact_id}", response_model=FamilyContactOut)
async def update_family_contact(
    ...
    row = await family_contacts_repo.update_family_contact(db, family_contact_id, **fields)
    if row is None:
    ...
    await admin_audit.record(
        ...
        entity_type="family_contact",
        entity_id=str(family_contact_id),
...
async def delete_family_contact(
    ...
    # Read first to capture the parent contact id for the audit row before deletion.
    existing = await family_contacts_repo.get_family_contact(db, family_contact_id)
    if existing is None:
        raise HTTPException(status_code=404, detail="family contact not found")
    parent_contact_id = existing.contact_id
    try:
        await family_contacts_repo.delete_family_contact(db, family_contact_id)
    ...
    await admin_audit.record(
        ...
        entity_type="family_contact",
        entity_id=str(family_contact_id),
        detail={"contact_id": str(parent_contact_id)},
    )
    await db.commit()
...
async def list_family_reports(
    ...
    rows = await family_reports_repo.list_reports(
        db, contact_id=contact_id, limit=limit, offset=offset
    )
    return [
        FamilyReportOut.model_validate(report).model_copy(update={"contact_name": name})
...
async def resend_family_report(
    ...
    # Re-enqueues the SAME PHI-free body to the contact's opted-in family contacts with no
    ...
    recipients = await family_contacts_repo.list_alert_recipients(
        db, contact_id=report.contact_id, kind="report"
    )
    if not recipients:
    ...
    await db.commit()
    contact = await contacts_repo.get_contact(db, report.contact_id)
    return FamilyReportOut.model_validate(report).model_copy(
        update={"contact_name": contact.name if contact is not None else None}
    )
