...
parsed ``requested_at`` in the past), clamps the dial time to the contact's quiet hours,
...
async def _truncate(session_factory):
    async with session_factory() as db:
        await db.execute(text("TRUNCATE callback_requests, calls, dnc_list, contacts CASCADE"))
        await db.commit()
...
async def _seed(
    ...
    *,
    ...
    """Create contact + a source call + a callback request; return (contact_id, callback_id)."""
    async with session_factory() as db:
        contact = await contacts_repo.create_contact(
            db, name="Ada", phone_e164=_phone(), timezone=tz
        )
        src = await calls_repo.create_call(
            db, contact_id=contact.id, direction=CallDirection.OUTBOUND, status=call_status
        )
        if on_dnc:
            await dnc_repo.add_entry(db, contact.phone_e164, "test opt-out")
...
async def _calls_for(session_factory, contact_id: uuid.UUID) -> list[Call]:
    async with session_factory() as db:
        rows = (
            await db.execute(
                select(Call).where(Call.contact_id == contact_id, Call.parent_call_id.is_(None))
            )
        ).scalars()
        return list(rows)
...
async def test_due_open_callback_materializes_queued_call(session_factory):
    ...
    dialed = await _calls_for(session_factory, contact_id)
...
async def test_quiet_hours_deferral(session_factory):
    ...
    contact_id, cb_id = await _seed(session_factory, tz="America/New_York", requested_at=req)

    await callback_dialer.poll_once(session_factory, _settings(), now=req)

    call = [
        c for c in await _calls_for(session_factory, contact_id) if c.status is CallStatus.QUEUED
    ][0]
...
async def test_dnc_blocks_callback(session_factory):
    ...
    blocked = [
        c
        for c in await _calls_for(session_factory, contact_id)
        if c.status is CallStatus.DNC_BLOCKED
    ]
...
async def test_future_callback_not_due(session_factory):
    contact_id, cb_id = await _seed(
        session_factory, tz="UTC", requested_at=NOW + timedelta(hours=2)
    )
...
async def test_null_requested_at_not_dialed(session_factory):
    ...
    # never auto-dialed.
    contact_id, cb_id = await _seed(session_factory, tz="UTC", requested_at=None)
