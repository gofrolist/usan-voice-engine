...
def test_create_call_request_rejects_sched_prefix() -> None:
    with pytest.raises(ValidationError) as exc_info:
        CreateCallRequest(contact_id=uuid.uuid4(), idempotency_key="sched:xyz")
...
def test_create_call_request_rejects_batch_prefix() -> None:
    with pytest.raises(ValidationError) as exc_info:
        CreateCallRequest(contact_id=uuid.uuid4(), idempotency_key="batch:1")
    assert "reserved" in str(exc_info.value)
...
def test_create_call_request_accepts_non_prefix_match() -> None:
    ...
    req = CreateCallRequest(contact_id=uuid.uuid4(), idempotency_key="scheduled-call-1")
