...
to the do-not-call list (FR-038) and — when the sender is a known contact — a one-time
...
async def _make_contact(session_factory, *, phone: str) -> str:
...
async def _make_contact_for_new_contact(session_factory, *, phone: str) -> str:
    """A family contact whose number is ``phone``, linked to a fresh contact."""
    async with session_factory() as db:
        contact_id = (
            await db.execute(
                text(
...
async def _tasks_for(session_factory, contact_id: str) -> list:
    async with session_factory() as db:
        rows = await db.execute(
            text("SELECT message FROM family_tasks WHERE contact_id = :e"),
            {"e": uuid.UUID(contact_id)},
        )
        return rows.all()
...
async def test_inbound_stop_from_contact_adds_dnc_and_ack(client, signer, session_factory):
    phone = _sender()
    await _make_contact(session_factory, phone=phone)
...
async def test_inbound_stop_from_known_contact_creates_no_task(client, signer, session_factory):
    ...
    phone = _sender()
    contact_id = await _make_contact_for_new_contact(session_factory, phone=phone)
    r = _post(client, signer, _envelope("stop_contact", "STOP", phone))
    assert r.status_code == 200, r.text
    assert await _dnc_blocked(session_factory, phone) is True
    assert await _tasks_for(session_factory, contact_id) == []
...
async def test_inbound_non_keyword_still_creates_task(client, signer, session_factory):
    ...
    rows = await _tasks_for(session_factory, contact_id)
    assert len(rows) == 1
...
async def test_inbound_stop_unknown_sender_adds_dnc_without_ack(client, signer, session_factory):
    ...
    # is no contact to attach an acknowledgement to.
    phone = _sender()
