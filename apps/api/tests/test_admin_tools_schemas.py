...
def test_followup_flag_summary_from_attributes():
    ...
    class _Row:
        ...
        # C3 contact identity: masked_phone is REQUIRED — computed by the router
...
def test_callback_request_summary_from_attributes():
    ...
    class _Row:
        ...
        call_id = uuid.uuid4()
        contact_id = uuid.uuid4()
        # C3 contact identity (masked_phone required; see flag stub note above).
        contact_name = None
        masked_phone = "***4567"
    ...
    assert summary.contact_id == row.contact_id
    assert summary.contact_name is None
