...
@pytest.mark.asyncio
async def test_serialize_registered_call_skips_metrics_fetch(app_session) -> None:
    """Pre-answer (REGISTERED) serialize_call never queries CallMetrics — behavior-preserving
    guard: there is no row to find, and _build_cost(None)/_token_usage(None) both return None
    regardless, so call_cost and llm_token_usage are absent in the output either way."""
    org_id = (await app_session.execute(text("SELECT id FROM organizations LIMIT 1"))).scalar_one()
    await set_tenant_context(app_session, org_id)
    ...
    call = Call(
        ...
        call_type=CallType.WEB_CALL,
        profile_override=profile.id,
        livekit_room="usan-web-guard-test",
        dynamic_vars={},
    )
    app_session.add(call)
    await app_session.flush()
    ...
    with patch.object(metrics_repo, "get_call_metrics", spy):
        out = await call_serializer.serialize_call(
            app_session, call, settings, client_host="1.2.3.4"
        )

    spy.assert_not_awaited()
    dumped = out.model_dump(exclude_none=True)
    assert "call_cost" not in dumped
    assert "llm_token_usage" not in dumped


@pytest.mark.asyncio
async def test_serialize_completed_call_fetches_metrics(app_session) -> None:
    """Terminal/answered calls still go through the metrics fetch path.
    Without an actual CallMetrics row the result is None (same as before), confirming
    the guard does not suppress metrics for post-answer statuses."""
    org_id = (await app_session.execute(text("SELECT id FROM organizations LIMIT 1"))).scalar_one()
    await set_tenant_context(app_session, org_id)
    ...
    call = Call(
        ...
        call_type=CallType.WEB_CALL,
        profile_override=profile.id,
        livekit_room="usan-web-completed-test",
        dynamic_vars={},
    )
    app_session.add(call)
    await app_session.flush()
    ...
    spy = AsyncMock(return_value=None)
    with patch.object(metrics_repo, "get_call_metrics", spy):
        await call_serializer.serialize_call(app_session, call, settings, client_host="1.2.3.4")
