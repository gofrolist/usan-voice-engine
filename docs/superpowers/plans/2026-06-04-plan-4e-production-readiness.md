...
- [x] **E1 (infra — PR-1):** `registry.tf` adds `google_artifact_registry_repository.usan` (Docker, `us-east1` → `us-east1-docker.pkg.dev/usan-retirement/usan`) + `artifactregistry.googleapis.com`, and grants the **VM SA** `roles/artifactregistry.reader`.
- [x] **E2 (infra — PR-1):** `registry.tf` adds a **WIF** pool (`github-pool`) + provider (`github-provider`, issuer `token.actions.githubusercontent.com`, `attribute_condition` pinned to `var.github_repository`) + deploy SA `github-deployer` with `roles/artifactregistry.writer` and a `workloadIdentityUser` binding scoped via `principalSet .../attribute.repository/<repo>`. Keyless — no SA key in GitHub secrets. Outputs `gar_repository` / `wif_provider` / `github_deployer_sa` for the cutover. `terraform plan` = 8 add / 0 change / 0 destroy (additive). **Apply before PR-2.**
...
- [ ] **E5 (cleanup — after a GAR deploy validates):** delete the `GHCR_PAT` GitHub secret and **revoke the PAT** (the VM no longer uses it; GHCR push uses `GITHUB_TOKEN`). README deploy-secrets note already updated. Optionally enable Artifact Analysis (vuln scanning).
