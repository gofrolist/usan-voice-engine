...
| `POST /v1/webhook-deliveries/{id}/redeliver` | Guarded SQL reset: `UPDATE … SET status='pending', attempts=0, next_attempt_at=now(), response_code=NULL, last_error=NULL, delivered_at=NULL WHERE id=:id AND status IN ('delivered','failed') RETURNING id` (`delivered_at` resets with the other diagnostics — a redelivered row that later terminally fails must not keep a contradictory `delivered_at`) — the status predicate is load-bearing (a Python check would race the poller's in-flight claim). 409 if the row is already `pending`. 429 if the target endpoint already has ≥100 pending deliveries (backpressure; also bounds re-arm storms from a leaked operator key — review L2). The `X-Usan-Delivery-Id`/body `delivery_id` stays stable — it identifies the event occurrence at that endpoint, and receiver-side dedupe on it is exactly what redelivery semantics want. 409 if the endpoint is disabled. 202 `{delivery_id}`. |
...
**Session-held-across-POST (design tradeoff, stated honestly — post-review):** `deliver_one` opens one DB session per row and holds it (and its transaction) open across the HTTP POST — the endpoint read, the POST, and the guarded outcome write share that per-row transaction. No row locks span the POST (the claim already committed; the endpoint read is lock-free), but one pooled connection per concurrently-delivering endpoint group sits idle-in-transaction for up to `WEBHOOK_DELIVERY_TIMEOUT_S` per row, sequential within the group. Bounded by design: at most 10 endpoint groups (the §8.4 cap) against the pool, accepted for the simplicity of the per-row commit discipline. Revisit if the endpoint cap is ever raised.
...
**Where the residual is live (security-review wording):** dev compose ships `WEBHOOK_DELIVERY_ENABLED` defaulting **on** (§11.1), so the rebind residual is live in every dev stack; prod ships the flag pinned **off**, carrying zero residual until the operator's enable sequence (§11.3). The IP-pinning transport (open Q2) remains the pre-scale-enable blocker.
