...
- **WAF (Free plan reality):**
  ...
  - Bot Fight Mode — **off by default.** On Free it is a global toggle that **cannot be path-exempted**, so it would challenge/break the server-to-server webhooks (`/v1/webhooks/*`); optional to enable post-verification, only after confirming a real webhook survives it.
  - **1 rate-limiting rule** — on `/v1/auth/*` (login-flood protection), the highest-value single rule.
  - **Custom WAF rules** (≤5 on Free) — proposed: (1) a **`skip` rule** on `/v1/webhooks/*` that bypasses the Free **managed ruleset** for Telnyx/LiveKit server-to-server webhooks (no browser to solve a Managed Challenge → would silently break inbound calls/callbacks), (2) block `/metrics` at the edge (defense-in-depth over the existing Caddy 403), (3) a basic "block obvious injection / known-bad path" rule; leave headroom for tuning.
...
| **Grafana JWT posture** (P5 security review): a removed CF Access user keeps a live Grafana session cookie until it expires (revocation lag); the local login form stays enabled as break-glass; `GF_AUTH_JWT_EXPECT_CLAIMS` must stay pinned to the app AUD (blank → fail-open). | Low/Medium | Accepted for the single-operator setup (the form is reachable only after passing CF Access + AOP; auto-signup is bounded by the Access allowlist). Documented in `docker-compose.monitoring.yml`. Post-GA: shorten Grafana session lifetime, consider `GF_AUTH_DISABLE_LOGIN_FORM` + a role claim. |
...
| **Cloudflare proxy / Bot Fight Mode can challenge server-to-server webhooks on `api.<domain>`** — the Telnyx/LiveKit `/v1/webhooks/*` POSTs are unattended machine-to-machine calls with no browser to solve a Managed Challenge; if challenged/blocked they fail silently, breaking inbound calls and telephony callbacks. | Medium | A **managed-ruleset `skip` rule** scoped to `/v1/webhooks/*` (§6.1) bypasses the Free managed ruleset for the webhook surface; **Bot Fight Mode left off by default** (Free cannot path-exempt it, so it would break webhooks); and an **explicit webhook cutover check** (a real inbound call's webhook completing end-to-end through Cloudflare, with the outcome recorded) is a required acceptance gate before declaring cutover done. |
