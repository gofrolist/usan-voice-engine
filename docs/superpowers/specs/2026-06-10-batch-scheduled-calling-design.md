...
- **Volume-abuse limits**: `MAX_BATCH_TARGETS=500` per batch (422 above), duplicate elders per batch rejected (422), `dynamic_vars` ≤8 KB per target, bounded list reads (≤500), the hard `MAX_CONCURRENT_CALLS` ceiling on dial rate, and `MAX_AUTONOMOUS_CALLS_PER_ELDER_PER_DAY` on per-elder repetition. A runaway operator script can fill tables but cannot accelerate dialing or harass an individual elder. `AUTONOMOUS_DIALING_PAUSED` is the state-preserving emergency stop (§5.4, §10).
...
4. **Emergency stop & rollback runbook** (in order of increasing severity):
   - *Pause* (reversible, state-preserving): set `AUTONOMOUS_DIALING_PAUSED=true` + `compose up` — every poller-claimed dial stops; in-flight calls finish; resume by flipping back.
   ...
   - *Stop materialization*: `SCHEDULER_POLLER_ENABLED=false` — note this does **not** stop already-materialized QUEUED rows, which ride the retry poller; pause or cancel for those.
