...
class MetricsAccumulator:
    ...
    def _start_turn(self) -> dict[str, Any]:
    ...
    def handle(self, ev: Any) -> None:
        ...
        if name == "EOUMetrics":
            ...
            turn["eou_delay_ms"] = _ms(getattr(m, "end_of_utterance_delay", None))
            turn["transcription_delay_ms"] = _ms(getattr(m, "transcription_delay", None))
        elif name == "STTMetrics":
            self.stt_audio_seconds += float(getattr(m, "audio_duration", 0.0) or 0.0)
            turn = self._ensure_turn()
            turn["stt_duration_ms"] = _ms(getattr(m, "duration", None))
            ...
            # Multiple LLM events per turn (e.g. tool-call then synthesis) keep the
            # last ttft/completion on the turn; the usage counters above accumulate all.
            turn = self._ensure_turn()
            turn["llm_ttft_ms"] = _ms(getattr(m, "ttft", None))
            turn["llm_completion_tokens"] = getattr(m, "completion_tokens", None)
        elif name == "TTSMetrics":
            self.tts_characters += int(getattr(m, "characters_count", 0) or 0)
            turn = self._ensure_turn()
            turn["tts_ttfb_ms"] = _ms(getattr(m, "ttfb", None))
