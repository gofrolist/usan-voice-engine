...
def _gcp_serialize(record: Any) -> str:
    ...
    Bound ``extra`` fields are promoted to top-level jsonPayload fields so Cloud
    Logging indexes them; the reserved keys set below always win over ``extra`` so a
    stray bound key can't shadow severity/message. Local variables are never
    serialized (matches diagnose=False) — only the formatted exception text.
    """
